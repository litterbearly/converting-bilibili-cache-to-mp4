...
class GUI:
    path_video = ["E:/Python/download"]
    out_path = 'E:/Python/bilibili1'
    ...
    def __init__(self):
        ...
        self.root.title("bilibili缓存转Mp4工具单线程")
    ...
    def convert(self, path_ffmpeg, in_path_list, out_path):
        for in_path in in_path_list:  # 遍历所有课程
            ...
            for i, path_entry_json in enumerate(self.path_entry_jsons):
                ...
                self.canvas.coords(fill_line, (0, 0, 500*i/len(self.path_entry_jsons), 60))
                text_progress_statement = "{:4.2f}%    {}/{}".format(100 * (i + 1) / len(self.path_entry_jsons),
                                                                       (i + 1), len(self.path_entry_jsons))
                ...
                try:
                    ...
                    self.wrong_list.append(path_entry_json)
        print("转换失败列表：", self.wrong_list)
