...
class GUI:
    ...
    def __init__(self):
        ...
        self.root.title("哔哩哔哩Cache缓存转Mp4工具单线程")
    ...
    def convert(self, path_ffmpeg, in_path_list, out_path):
        ...
        for num in range(len(all_command_text)):
            # 转换进度百分比展示
            text_progress_statement = "{:4.2f}%    {:2d}/{}".\
                format(100 * (num + 1) / len(all_command_text), (num + 1), len(all_command_text))
            ...
            # 当前转换文件名
            tk.Label(self.root,
                     text=all_classes_name[num][:10] + " " * 5 + all_class_name[num][:15],
                     font=self.font_style).place(x=100, y=550)  # 输入框，标记，按键

            # 转换命令
            os.system(all_command_text[num])
        end_time = time.time()
